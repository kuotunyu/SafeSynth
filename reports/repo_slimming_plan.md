...
> Generated by a script, not typed by hand. Regenerate any time; it reads
> `git ls-files` and every tracked `.md`, and changes nothing.
...
| everything else (code, docs, configs, results) | 40.9 MB | 6% |

A clone downloads history, not the working tree, so deleting these at HEAD
would change nothing: the packed object store is **631.3 MiB** today.
Rewriting history is the only thing that shrinks it, and this repo has never
been pushed - `git remote -v` is empty - so doing it now costs nothing and
doing it later means force-pushing over published history.

## KEEP - a document points at these

31 files. Every one is reachable by a reader following a link in a
tracked `.md`.
...
- `reports/figures/headline.png` (0.10 MB)
...
**Subtotal: 56.6 MB.** Four of these are 8-10 MB contact
sheets (`h4_*`); downsizing them is a separate, optional step worth about
37 MB.

## DROP - no document points at these

118 files. Their only mention anywhere is inside the script that
generated them, which is not a reference a reader can follow. Almost all are
Phase 1 diagnostics from abandoned routes: `supervised_labeler_v12` through
`v23` audits at ~18 MB per version, plus FLUX.2 and paired-person preflights.

**The evidence they represent is not being discarded** - the worklog records
which 18 labeler iterations and 9 synthesis routes were tried and what each
returned. That is the form a reader can actually search.
...
That removes the WHOLE folder from history, including the keepers. Re-add
them afterwards as one fresh commit:

```bash
git add reports/figures/ && git commit -m "docs: restore the figures documents reference"
```

The keepers survive the rewrite because they are still in the working tree -
`filter-repo` edits history, not your files on disk.
...
`size-pack` should fall from 631 MiB to roughly 100 MB. Then:

```bash
uv run pytest -q && uv run python scripts/verify_readme.py
```

`verify_readme` is the real check: it fails if any README figure link is dead.
