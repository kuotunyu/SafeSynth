...
**判定式**：`object_mean_luma >= min_object_mean_luma[class_name]`。
`object_mean_luma` 是**物件遮罩內**（不是框內）像素的 BT.601 亮度平均。
`kind == "existing"` 的標註改用 `min(門檻, object_mean_luma_pre_postfx)`。

**參數**：`configs/filtering.yaml` → `rules.annotation_legibility.min_object_mean_luma`
（每類一個值，本文件不複製數值）
**實作位置**：`src/filtering/rules.py` 的 `_filt_15`；量測在
`src/synthetic/composition.py` 的 `object_legibility`
**拒絕原因**：`ILLEGIBLE_ANNOTATION`
**驗證**：`uv run pytest tests/test_rules.py -k filt_15`
**狀態**：[x] 實作並校準完成

**為什麼需要這條**：使用者審查 `preview_head_no_helmet_p1` 第 02 格，指出
「有 head 框但物件是一團黑根本看不到」。追查發現低光 post-fx 把整張圖壓成近乎全黑，
而框照樣留著——等於告訴偵測器「這片黑色矩形裡有一頂安全帽」。這是標籤雜訊，不是難例。

**三個設計決定，都由量測而非直覺決定**：

1. **量物件遮罩，不量框**。框的四角混進背景：那顆被投訴的頭在框上量到 67.9，
   在自己的像素上只有 32.2。只有後者描述那顆頭。
2. **門檻分類別**。真實 head 的物件亮度 p1 = 23.19，helmet 是 45.58——
   深色頭髮讓裸頭本來就比安全帽暗得多。共用一個門檻會**優先砍掉深色裸頭**，
   而那正是整條管線存在的理由。
3. **兩個曾被考慮的統計量已量測後否決**，記在這裡以免再被提出：
   - **框內 RMS 對比**：與框面積強相關（真實 p1 在 16² 以下是 13.21，96² 以上是 30.75），
     且懲罰「框得準」——貼上的 helmet 框比真實框貼身，同一統計量低約 20 點。
     實測它誤殺了又小又亮、目視完全清楚的遠距安全帽
   - **|物件 − 周圍| 亮度差**：真實物件幾乎沒有。同一批 9,761 個標註 p1 = 0.67、p5 = 3.34，
     因為真實物件與背景的差異主要在**色相**而非亮度。任何咬得動的門檻都會誤殺真實構圖

**⚠️ 這條規則有一個結構性盲點，由 [CUT-14](synthesis_spec.md) 補**：
它量的是**合成結果**，而 Lab 調和在它之前執行。
一個素材亮度只有 8.5 的純黑剪影，被調和抬到 45.4 就能通過本規則，
但它仍然是一塊沒有特徵的黑斑。**調和搬動平均值，不會生出細節。**
因此素材端必須另有一道同門檻的閘門，兩條規則不是重複。

**已知範圍**：這條規則抓不到「暗頭髮貼到暗背景」（原始投訴的第 01 格）。
那顆頭在所有可量指標上都落在真實分布內（素材亮度 32.2、物件亮度 32.2，
真實 head p1 = 23.19、p2 = 29.17），CUT-14 也不會擋它。
把門檻拉高到足以抓它，就會連同真實資料裡最暗的那批裸頭一起丟掉。
...
不合理就先修，不要把明顯有問題的圖交給 reviewer。每次人工審查都要明列需要查看的
圖片、判斷規則與可直接回填的回饋格式。
