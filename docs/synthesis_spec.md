...
**COMP-20** — 素材以**挖料為主、程序生成為輔**（[ADR-004](decisions.md#adr-004)）。

- **挖料**：對 Train 影像中**沒有任何標註**的區域，用 HSV 安全黃／橙窗 ＋ 輪廓面積 ＋ 圓度篩選，
  再用該輪廓的外接框 prompt SAM2 取得乾淨 cutout。
  拿到的是真正的挖土機面板、三角錐、油桶、電纜捲盤——**帶著本域真實的材質與光照統計**
- **程序生成**：安全色系的圓頂／橢圓／圓弧／管件，帶高光與邊緣陰影。
  **必須用陰影調變真實背景紋理，不可平坦填色**——否則偵測器會學到
  「程序紋理 ⇒ 負樣本」而非「黃色圓形物 ⇒ 負樣本」，那是**簡單**負樣本，與目的相反
