...
### K-11 — Hard negative 看起來像貼上的色塊，不像場景中的物件

**症狀**：`reports/figures/preview_hard_negative.png` 裡的 distractor 明顯「浮」在畫面上——
沒有接地陰影、懸在任意深度（例如浮在工人胸前或樹冠上方）、
光照與所在場景不一致，整體像貼紙而非實物。

**根因**：目前的 hard negative 合成通道只做了幾何變換與 alpha 合成，
**沒有做接地陰影、也沒有依深度調整尺寸**。程序生成的素材本身是用真實背景紋理調變的
（[ADR-012](decisions.md#adr-012)），但貼上之後缺少讓它「落在場景裡」的線索。

**影響**：
- 這些 distractor 偏向 *easy* negative。偵測器可能學到「浮著的平滑色塊 ⇒ 背景」，
  而不是我們要的「黃色圓形物 ⇒ 不是安全帽」，因此
  **`hard-negative 每圖誤報數` 這個指標測到的東西比預期弱**
- 它也直接餵養 H4 的貼上痕跡可偵測度

**目前的處置**：**不修，列為已知限制。**
[ADR-011](decisions.md#adr-011) 已經接受「貼上痕跡可被偵測」是本專案的公開發現，
而 distractor 的真實度問題是同一個限制的子集。
要真正修好需要接地陰影估計與深度感知的尺寸調整，那是另一個專案規模的工作。

**若日後要修**，順序應該是：接地陰影 → 依 `cy` 的深度尺寸先驗 → 局部光照方向匹配。
修完必須重跑 H4 並比較 AUC 是否下降，否則無法證明有效。
