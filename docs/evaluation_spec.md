...
**EVAL-07 — `AP_small` 的面積門檻必須在「原始 416×416 座標」下計算。**

COCO 的 `AP_small` 定義是 `area < 32² = 1024 px²`。
但我們**在 416×416 的原生解析度標註、在 640×640 訓練**。
若把預測與 GT 都放大到 640 再算面積，同一個物件的面積會變成 `(640/416)² ≈ 2.37` 倍，
**大量原本屬於 small 的物件會被歸到 medium**，`AP_small` 就變成在測量另一件事。

規則：
- COCO GT JSON 的 `area` 欄位一律是**原始 416×416 座標**下的 `w * h`
