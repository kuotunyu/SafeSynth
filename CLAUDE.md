...
- >2GB 下載或任何花錢動作**先問使用者**
- **任何超過 1 小時的作業，交付前先講三件事**：①總時數（**必須來自實測，不准用「大概慢 N 倍」推算**）
  ②中途需不需要人看著 ③**「你現在應該去做別的事／去睡覺」**。
  只講時數不算——使用者會照著你的語氣決定要不要等。
  一份 7 小時的工作若沒明說「這是過夜的」，等於叫人熬夜盯著進度條。
  **估不出來就說估不出來**，並給一個「先跑 X 分鐘回報實測速度」的做法
...
| 資料集 | Hard Hat Workers（Kaggle `andrewmvd/hard-hat-detection`，**CC0 1.0**），5,000 張 PNG |
| 解析度 | ⚠️ **不是單一解析度**：416×415 有 2,461 張（**多數**）／416×416 有 2,192／415×416 有 324／415×415 有 23。**預測框一律逐圖用該圖自己的寬高映射回原座標，絕不可用一個全域縮放因子**（實測於 `coco_all.json` 並抽 40 張 PNG 對照，零不符） |
...
2. ~~`M11` 之前合成總量不得超過 300 張~~ → **已由 [ADR-011](docs/decisions.md#adr-011) 改寫**：
   H4 **未通過**。最新實測 AUC **0.9053**（95% CI 0.9013–0.9090，106,144 個 patch，
   [ADR-013](docs/decisions.md#adr-013) 重生成後的 pool），預先登記上限 0.60。
   歷程：300 張時 0.7964 → 舊 1× pool 0.9159 → 現在 0.9053。
   此判定不變、也不得宣稱通過。
   但後果從「無限阻擋」改為「**允許到 1×，禁止 2×**」，並把 H4 失敗當成已知限制帶進 Phase 2。
   **所有成果表都必須與 H4 AUC 並列呈現**，README 正文要寫出這件事。
