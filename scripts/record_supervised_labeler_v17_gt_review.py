"""Record the v17 GT-only owner review and freeze its clean audit."""
...
OWNER_REVIEW_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v17_gt_owner_review.json"
)
AUDIT_PATH = (
...
PROBLEMS = {
    10: {
        ...
        "decision": "AMBIGUOUS",
        "reason": (
            "The extremely flat target at the top image edge is too small and "
            "incomplete to require a meaningful worn-helmeted-head box."
        ),
    },
    21: {
        "image_id": 1767,
        "decision": "AMBIGUOUS",
        "reason": (
            "The two far-left targets are narrow clipped edge fragments. They "
            "are too incomplete to require meaningful worn-helmeted-head boxes."
        ),
    },
}
    ...
    *,
    ...
    for case in evidence["cases"]:
        ...
        if problem is not None and image_id != int(problem["image_id"]):
            raise RuntimeError(f"v17 problem cell {cell:02d} identity changed")
        decisions.append(
            {
                "cell": cell,
    ...
    review = {
        "schema_version": 1,
        "status": "v17_gt_only_primary_adjudicated_two_quarantines",
        ...
        "decisions": decisions,
        "owner_note": (
            "Quarantine complete images 10 and 21 because their tiny clipped "
            "edge fragments do not require boxes. All other 62 primary GT-only "
            "cells are approved."
        ),
...
def build_adjudicated_audit(
    *,
    ...
    if Counter(decisions.values()) != Counter({"PASS": 62, "AMBIGUOUS": 2}):
        raise RuntimeError("v17 owner review decisions changed")
    ...
    if dict(selected_per_stratum) != quotas:
        raise RuntimeError("Valid v17 primary cases do not satisfy quotas")
    ...
    manifest = {
        "schema_version": 1,
        "status": "v17_adjudicated_audit_frozen_before_training",
        "experiment_id": str(pool["experiment_id"]),
        "source_split": "Train",
    ...
    if (
        ...
        raise RuntimeError("v17 adjudicated audit violates frozen boundaries")
...
def main() -> None:
    ...
    if cells != set(PROBLEMS):
        raise RuntimeError("Explicit --quarantine-cells 10,21 is required")
    if OWNER_REVIEW_PATH.exists() or AUDIT_PATH.exists():
        raise RuntimeError("v17 GT-only owner evidence already exists")
