"""Record kuotunyu's decision on exact frozen v10 review evidence."""
...
REVIEW_MANIFEST_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v10_review_manifest.json"
)
...
def _record(path: Path) -> dict[str, str]:
    if not path.is_file():
        raise RuntimeError(f"Frozen v10 review evidence is missing: {path}")
    return {
...
def build_v10_registration(
    *,
    config: dict[str, Any],
    split: dict[str, Any],
    report: dict[str, Any],
) -> dict[str, Any]:
    """Construct and verify the exact v10 numeric-pass review registration."""

    if (
        config.get("experiment_id") != "supervised_labeler_v10"
        ...
        raise RuntimeError("v10 must pass every numeric gate before review")
    ...
    if (
        ...
        raise RuntimeError("Exact v10 audit box evidence changed")
    registration = {
...
def main() -> None:
    ...
    fresh_registration = build_v10_registration(
        config=config,
        split=split,
        report=report,
    )
    if not REVIEW_MANIFEST_PATH.is_file():
        raise RuntimeError("Frozen v10 owner-review manifest is missing")
    ...
    if (
        ...
        or review_manifest.get("status") != "v10_owner_review_files_frozen"
        or review_manifest.get("registration") != fresh_registration
    ):
        raise RuntimeError("Frozen v10 owner-review manifest changed")
