"""Freeze the v8 Train-only audit after the v7 audit was consumed."""
...
V7_SPLIT_PATH = PROJECT_ROOT / "splits" / "supervised_labeler_v7_split.json"
REPORT_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v8_split.md"


def main() -> None:
    if SPLIT_PATH.exists() or REPORT_PATH.exists():
        raise RuntimeError("Supervised labeler v8 split is already frozen")
    config = load_supervised_labeler_config(CONFIG_PATH)
    if config["experiment_id"] != "supervised_labeler_v8":
        raise RuntimeError("Active supervised registration is not v8")
    v7_split = json.loads(V7_SPLIT_PATH.read_text(encoding="utf-8"))
    prior_calibration_ids = [
        int(value) for value in v7_split["calibration_image_ids"]
    ]
    prior_revealed_audit_ids = [
        int(value) for value in v7_split["untouched_audit_image_ids"]
    ]
    ...
    if set(payload["calibration_image_ids"]) != expected_calibration:
        raise AssertionError("v8 calibration did not consume the full v7 history")
    if set(payload["untouched_audit_image_ids"]) & expected_calibration:
        raise AssertionError("A revealed image entered the v8 audit")
    selected = (
    ...
    if test_ids & selected:
        raise AssertionError("Validation/Test leakage entered v8 split")
    ...
    lines = [
        "# Supervised labeler v8 Train-only split",
        "",
        ...
        (
            ...
            "(all previously consumed v1-v7 calibration/audit images)"
        ),
        f"- New sealed audit: **{payload['untouched_audit_images']} images**",
        "- Group overlap: **0**",
        "- Validation/Test images read: **0 / 0**",
        "- New audit pixels/metrics inspected before v8 training: **no**",
        "",
    ]
