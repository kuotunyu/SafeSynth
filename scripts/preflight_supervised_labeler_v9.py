"""Verify the sealed v9 registration without reading images or using GPU."""
...
REPORT_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v9_preflight.json"
MARKDOWN_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v9_preflight.md"


def main() -> None:
    if REPORT_PATH.exists() or MARKDOWN_PATH.exists():
        raise RuntimeError("Supervised labeler v9 preflight already exists")
    config = load_supervised_labeler_config(CONFIG_PATH)
    split = json.loads(SPLIT_PATH.read_text(encoding="utf-8"))
    if (
        config["experiment_id"] != "supervised_labeler_v9"
        or config["split_manifest_sha256"] != split["manifest_sha256"]
        or split["status"] != "frozen_before_supervised_training"
    ):
        raise RuntimeError("v9 registration and sealed split disagree")
    ...
    if test_ids & selected:
        raise RuntimeError("Validation/Test leakage entered v9")
    group_sets = [
    ...
    if any(
        ...
        raise RuntimeError("v9 source groups overlap")
    if any(int(image_id) not in train_images for image_id in selected):
        raise RuntimeError("v9 contains a non-Train image")
    ...
    if len(weights) != int(split["training_images"]):
        raise RuntimeError("v9 sampling weights do not cover training")
    if set(weights) - {1.0, 2.0}:
        raise RuntimeError("v9 sampling produced an unregistered weight")
    ...
    lines = [
        "# Supervised labeler v9 CPU preflight",
        "",
