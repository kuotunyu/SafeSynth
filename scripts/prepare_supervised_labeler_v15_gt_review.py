"""Freeze and render the independent v15 GT-only review pool."""
...
CONFIG_PATH = (
    PROJECT_ROOT / "configs" / "supervised_labeler_v15_gt_review.yaml"
)
POOL_PATH = PROJECT_ROOT / "splits" / "supervised_labeler_v15_gt_pool.json"
EVIDENCE_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v15_gt_review.json"
)
FIGURE_DIR = (
    PROJECT_ROOT
    / "reports"
...
def _verified_pool(path: Path) -> dict[str, Any]:
    ...
    if (
        ...
        or "gt_only_pool_frozen_before_pixel_review" not in str(pool["status"])
    ):
        raise RuntimeError(f"Frozen pool changed: {path}")
    return pool
...
def freeze_pool() -> dict[str, Any]:
    """Freeze 64 primary and 32 reserve cases without decoding pixels."""
...
def _render_pages(
    ...
    for page_index in range(4):
        ...
        draw.text(
            (16, 49),
            "GREEN = DATASET GT. NO MODEL BOXES APPEAR ON THESE PAGES.",
            fill=(0, 80, 0),
            font=_font(20, bold=True),
        )
        draw.text(
            (16, 80),
            "BOX = WORN HARD HAT + WEARER HEAD. LOOSE/UNWORN HAT = NO BOX.",
            fill="black",
            font=_font(18),
        )
        draw.text(
            (16, 111),
            (
                "REPORT: MISSING WORN HEAD, BOX ON BACKGROUND/LOOSE HAT, "
                "MERGED PEOPLE, OR BAD BOX."
            ),
            fill=(120, 0, 0),
        ...
        for local_index in range(16):
            ...
            draw.text(
                ...
                (
                    f"{case['cell']:02d} | Train image "
                    f"{case['image_id']} | green boxes "
                    f"{len(case['truth_boxes'])}"
                ),
                fill="black",
                font=_font(17),
            )
