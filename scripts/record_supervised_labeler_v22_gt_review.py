"""Record the v22 GT-only owner review and freeze its clean audit."""
...
OWNER_REVIEW_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v22_gt_owner_review.json"
)
AUDIT_PATH = (
...
PROBLEMS = {
    17: {
        ...
        "reason": (
            "The far-left green box is an extremely thin edge fragment; the "
            "helmeted head is too small and clipped for a meaningful box."
        ),
    },
    32: {
        "image_id": 869,
        ...
        "decision": "DATASET_GT_FALSE_POSITIVE",
        "reason": (
            "The right green box covers a partly hidden person without a "
            "visible safety hard hat."
        ),
    },
    33: {
        "image_id": 3775,
        ...
        "decision": "DATASET_GT_FALSE_POSITIVE",
        "reason": (
            "The far-right green box is an extremely thin edge fragment; the "
            "object is too small and clipped for a meaningful box."
        ),
    },
    41: {
        "image_id": 1108,
...
def build_owner_review(
    *,
    ...
    """Build the exact four-quarantine decision over all 64 primary cases."""
    ...
    for case in evidence["cases"]:
        ...
        if problem is not None and (
            ...
            raise RuntimeError(f"v22 problem cell {cell:02d} identity changed")
        decisions.append(
            {
                "cell": cell,
                "group_id": group_id,
    ...
    review = {
        "schema_version": 1,
        "status": "v22_gt_only_primary_adjudicated_four_quarantines",
        ...
        "categories": {
            "dataset_gt_false_positive_cells": [17, 32, 33, 41],
        ...
        "decisions": decisions,
        "owner_note": (
            "Quarantine complete images 17, 33, and 41 because their clipped "
            "edge boxes are too small or thin to be meaningful. Quarantine "
            "complete image 32 because its right box has no visible hard hat. "
            "All other 60 primary GT-only cells are approved."
        ),
...
def build_adjudicated_audit(
    *,
    ...
    if Counter(decisions.values()) != Counter(
        {"PASS": 60, "DATASET_GT_FALSE_POSITIVE": 4}
    ):
        raise RuntimeError("v22 owner review decisions changed")
    ...
    if dict(selected_per_stratum) != quotas:
        raise RuntimeError("Valid v22 primary cases do not satisfy quotas")
    ...
    manifest = {
        "schema_version": 1,
        "status": "v22_adjudicated_audit_frozen_before_training",
        "experiment_id": str(pool["experiment_id"]),
        "source_split": "Train",
    ...
    if (
        ...
        or len(surplus) != 12
        or len(quarantined) != 4
        or {int(row["primary_cell"]) for row in quarantined}
        != set(PROBLEMS)
    ):
        raise RuntimeError("v22 adjudicated audit violates frozen boundaries")
...
def main() -> None:
    ...
    parser.add_argument(
        "--quarantine-cells",
        required=True,
        help="Must be exactly 17,32,33,41.",
    )
    ...
    if cells != set(PROBLEMS):
        raise RuntimeError(
            "Explicit --quarantine-cells 17,32,33,41 is required"
        )
    if OWNER_REVIEW_PATH.exists() or AUDIT_PATH.exists():
        raise RuntimeError("v22 GT-only owner evidence already exists")

    config = yaml.safe_load(CONFIG_PATH.read_text(encoding="utf-8"))
    if config["status"] != "gt_only_primary_review_pending_owner":
        raise RuntimeError("v22 GT-only owner review is not pending")
    pool = _verified_json(
        POOL_PATH,
