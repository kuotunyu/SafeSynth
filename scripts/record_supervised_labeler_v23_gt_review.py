"""Record the v23 GT-only owner review and freeze its clean audit."""
...
from scripts.prepare_supervised_labeler_v23_gt_review import (
    CONFIG_PATH,
    EVIDENCE_PATH,
    POOL_PATH,
)
...
PROBLEMS = {
    5: {
        ...
        "reason": (
            ...
            "object is too small to identify as a worn helmeted head."
        ),
    },
    18: {
        "image_id": 233,
    ...
    *,
    ...
    for case in evidence["cases"]:
        ...
        if problem is not None and (
            ...
            raise RuntimeError(f"v23 problem cell {cell:02d} identity changed")
        decisions.append(
            {
                "cell": cell,
                "group_id": group_id,
    ...
    review = {
        "schema_version": 1,
        "status": "v23_gt_only_primary_adjudicated_three_quarantines",
        ...
        "decisions": decisions,
        "owner_note": (
            "Quarantine complete images 05, 18, and 52 because their "
            "far-left green boxes are extremely thin and the objects are too "
            "small to identify as worn helmeted heads. All other 61 primary "
...
def build_adjudicated_audit(
    *,
    ...
    if Counter(decisions.values()) != Counter(
        ...
        raise RuntimeError("v23 owner review decisions changed")
    ...
    if dict(selected_per_stratum) != quotas:
        raise RuntimeError("Valid v23 primary cases do not satisfy quotas")
    ...
    manifest = {
        "schema_version": 1,
        "status": "v23_adjudicated_audit_frozen_before_training",
        "experiment_id": str(pool["experiment_id"]),
        "source_split": "Train",
    ...
    if (
        ...
        raise RuntimeError("v23 adjudicated audit violates frozen boundaries")
...
def main() -> None:
    ...
    if cells != set(PROBLEMS):
        raise RuntimeError("Explicit --quarantine-cells 5,18,52 is required")
    if OWNER_REVIEW_PATH.exists() or AUDIT_PATH.exists():
        raise RuntimeError("v23 GT-only owner evidence already exists")

    config = yaml.safe_load(CONFIG_PATH.read_text(encoding="utf-8"))
    if config["status"] != "gt_only_primary_review_pending_owner":
        raise RuntimeError("v23 GT-only owner review is not pending")
    pool = _verified_json(
        POOL_PATH,
