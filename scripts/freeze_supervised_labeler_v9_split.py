"""Freeze the v9 Train-only audit after the v8 audit was consumed."""
...
V8_SPLIT_PATH = PROJECT_ROOT / "splits" / "supervised_labeler_v8_split.json"
REPORT_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v9_split.md"


def main() -> None:
    if SPLIT_PATH.exists() or REPORT_PATH.exists():
        raise RuntimeError("Supervised labeler v9 split is already frozen")
    config = load_supervised_labeler_config(CONFIG_PATH)
    if config["experiment_id"] != "supervised_labeler_v9":
        raise RuntimeError("Active supervised registration is not v9")
    v8_split = json.loads(V8_SPLIT_PATH.read_text(encoding="utf-8"))
    prior_calibration_ids = [
        int(value) for value in v8_split["calibration_image_ids"]
    ]
    prior_revealed_audit_ids = [
        int(value) for value in v8_split["untouched_audit_image_ids"]
    ]
    ...
    if set(payload["calibration_image_ids"]) != expected_calibration:
        raise AssertionError("v9 calibration did not consume the full v8 history")
    if set(payload["untouched_audit_image_ids"]) & expected_calibration:
        raise AssertionError("A revealed image entered the v9 audit")
    selected = (
    ...
    if test_ids & selected:
        raise AssertionError("Validation/Test leakage entered v9 split")
    ...
    lines = [
        "# Supervised labeler v9 Train-only split",
        "",
        ...
        (
            ...
            "(all previously consumed v1-v8 calibration/audit images)"
        ),
        f"- New sealed audit: **{payload['untouched_audit_images']} images**",
        "- Group overlap: **0**",
        "- Validation/Test images read: **0 / 0**",
        "- New audit pixels/metrics inspected before v9 training: **no**",
        "",
    ]
