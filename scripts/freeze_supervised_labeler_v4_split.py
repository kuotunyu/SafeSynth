"""Freeze the v4 Train-only audit after the v3 audit was consumed."""
...
V3_SPLIT_PATH = PROJECT_ROOT / "splits" / "supervised_labeler_v3_split.json"
REPORT_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v4_split.md"


def main() -> None:
    if SPLIT_PATH.exists() or REPORT_PATH.exists():
        raise RuntimeError("Supervised labeler v4 split is already frozen")
    config = load_supervised_labeler_config()
    v3_split = json.loads(V3_SPLIT_PATH.read_text(encoding="utf-8"))
    prior_calibration_ids = [
        int(value) for value in v3_split["calibration_image_ids"]
    ]
    prior_failed_audit_ids = [
        int(value) for value in v3_split["untouched_audit_image_ids"]
    ]
    ...
    if set(payload["calibration_image_ids"]) != expected_calibration:
        raise AssertionError("v4 calibration did not consume the full v3 history")
    selected = (
    ...
    if test_ids & selected:
        raise AssertionError("Validation/Test leakage entered v4 split")
    ...
    lines = [
        "# Supervised labeler v4 Train-only split",
        "",
        ...
        (
            ...
            "(all previously consumed v1/v2/v3 calibration/audit images)"
        ),
        f"- New untouched audit: **{payload['untouched_audit_images']} images**",
        "- Group overlap: **0**",
        "- Validation/Test images read: **0 / 0**",
        "- New audit pixels/metrics inspected before v4 training: **no**",
        "",
    ]
