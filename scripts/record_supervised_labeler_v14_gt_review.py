"""Record v14 GT review and freeze the clean independent audit."""
...
def build_owner_review(
    *,
    ...
    if (
        ...
        or evidence["v14_training_started"] is not False
        or int(evidence["sealed_reserve_pixels_read"]) != 0
        or int(evidence["validation_images_read"]) != 0
        or int(evidence["test_images_read"]) != 0
    ):
        raise RuntimeError("v14 GT-only evidence boundary changed")

    decisions = []
    for case in evidence["cases"]:
        cell = int(case["cell"])
        image_id = int(case["image_id"])
        if cell == PROBLEM_CELL and image_id != PROBLEM_IMAGE_ID:
            raise RuntimeError("v14 cell 60 identity changed")
        is_problem = cell == PROBLEM_CELL
        decisions.append(
            {
                "cell": cell,
                ...
                "decision": (
                    "DATASET_GT_FALSE_POSITIVE" if is_problem else "PASS"
                ),
                "reason": (
                    "The far-left green box covers an unidentifiable tiny "
                    "image-edge fragment that should not be forced as a "
                    "worn-helmeted-head annotation."
                    if is_problem
                    else None
                ),
            }
    ...
    review = {
        "schema_version": 1,
        "status": "v14_gt_only_primary_adjudicated_one_quarantine",
        ...
        "categories": {
            "dataset_gt_false_positive_cells": [PROBLEM_CELL],
        ...
        "decisions": decisions,
        "owner_note": (
            "Most v14 GT boxes were approved. In cell 60, the far-left green "
            "box is too small and unclear to retain; quarantine the complete "
            "image under the preregistered policy."
        ),
...
def build_adjudicated_audit(
    *,
    ...
    """Select first valid per-stratum cases while quarantining cell 60."""
    ...
    if Counter(decisions.values()) != Counter(
        {"PASS": 63, "DATASET_GT_FALSE_POSITIVE": 1}
    ):
        raise RuntimeError("v14 owner review decisions changed")
    evidence_by_cell = {
    ...
    if dict(selected_per_stratum) != quotas:
        raise RuntimeError("Valid v14 primary cases do not satisfy quotas")
    ...
    manifest = {
        "schema_version": 1,
        "status": "v14_adjudicated_audit_frozen_before_training",
        "experiment_id": str(pool["experiment_id"]),
        "source_split": "Train",
    ...
    if (
        ...
        or int(quarantined[0]["primary_cell"]) != PROBLEM_CELL
    ):
        raise RuntimeError("v14 adjudicated audit violates frozen boundaries")
...
def main() -> None:
    ...
    parser.add_argument(
        "--quarantine-cell-60",
        action="store_true",
        help="Record the owner's explicit cell-60 quarantine decision.",
    )
    args = parser.parse_args()
    if not args.quarantine_cell_60:
        raise RuntimeError("Explicit --quarantine-cell-60 is required")
    if OWNER_REVIEW_PATH.exists() or AUDIT_PATH.exists():
        raise RuntimeError("v14 GT-only owner evidence already exists")
