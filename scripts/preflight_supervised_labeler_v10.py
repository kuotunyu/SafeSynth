"""Verify v10 normalization on unsealed Train-only partitions without GPU."""
...
REPORT_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v10_preflight.json"
MARKDOWN_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v10_preflight.md"
V9_PROBLEM_IMAGE_IDS = {345, 1027, 1124, 3569}
...
def _scan_dataset(
    *,
    ...
    for index, image_id in enumerate(dataset.image_ids):
        ...
        if image_id in V9_PROBLEM_IMAGE_IDS:
            problem_images[str(image_id)] = {
                "partition": name,
                "source_helmet_annotations": source_count,
                "transformed_helmet_annotations": transformed_count,
                **normalization,
...
def main() -> None:
    if REPORT_PATH.exists() or MARKDOWN_PATH.exists():
        raise RuntimeError("Supervised labeler v10 preflight already exists")
    config = load_supervised_labeler_config(CONFIG_PATH)
    split = json.loads(SPLIT_PATH.read_text(encoding="utf-8"))
    if (
        config["experiment_id"] != "supervised_labeler_v10"
        or config["split_manifest_sha256"] != split["manifest_sha256"]
        or split["status"] != "frozen_before_supervised_training"
    ):
        raise RuntimeError("v10 registration and sealed split disagree")
    (
        _,
        _,
        paths,
    ...
    if test_ids & selected:
        raise RuntimeError("Validation/Test leakage entered v10")
    group_sets = [
    ...
    if any(
        ...
        raise RuntimeError("v10 source groups overlap")
    ...
    problem_images = {
        **training_scan.pop("problem_images"),
        **calibration_scan.pop("problem_images"),
    }
    if set(problem_images) != {str(value) for value in V9_PROBLEM_IMAGE_IDS}:
        raise RuntimeError("v9 revealed problem images were not all normalized")
    if any(not record["applied"] for record in problem_images.values()):
        raise RuntimeError("A v9 problem image escaped reflection normalization")
    ...
    payload = {
        ...
        "v9_problem_images": problem_images,
    ...
    lines = [
        ...
        (
            "- Removed mirrored/clipped helmet boxes "
            f"(training / calibration): "
            f"**{training_scan['removed_helmet_annotations']} / "
            f"{calibration_scan['removed_helmet_annotations']}**"
        ),
