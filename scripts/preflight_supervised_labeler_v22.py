"""Verify v22 partitions, normalization, replay, and base model on CPU."""
...
REPORT_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v22_preflight.json"
MARKDOWN_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v22_preflight.md"
)


def main() -> None:
    if REPORT_PATH.exists() or MARKDOWN_PATH.exists():
        raise RuntimeError("Supervised labeler v22 preflight already exists")
    ...
    if (
        ...
        raise RuntimeError("v22 registration and frozen split disagree")
    ...
    v22_groups = set(split["v22_reserved_group_ids"])
    v21_revealed_groups = set(split["v21_revealed_audit_group_ids"])
    v21_nonselected_groups = set(
    ...
    prior_sealed_groups = {
        ...
        and key != "v21_nonselected_excluded_group_ids"
    ...
    if (
        ...
        or model_groups & v22_groups
        or model_groups & v21_nonselected_groups
        or model_groups & prior_sealed_groups
        or not v21_revealed_groups <= training_groups
        or not replay_ids <= set(split["training_image_ids"])
        or len(audit) != 48
    ):
        raise RuntimeError("v22 model-data independence changed")
    ...
    if not set(replay_weights.values()) <= {12.0, 28.0, 40.0}:
        raise RuntimeError("v22 error-replay weights changed")
    ...
    if (
        ...
        raise RuntimeError("v22 replay maximum-weight policy changed")
    ...
    payload = {
        ...
        "v22_reserved_groups_in_model_data": 0,
        "v21_nonselected_groups_in_model_data": 0,
        "prior_sealed_groups_in_model_data": 0,
        "v21_revealed_audit_groups_in_training": len(v21_revealed_groups),
