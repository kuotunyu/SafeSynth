"""Verify v18 partitions, normalization, replay, and base model on CPU."""
...
REPORT_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v18_preflight.json"
MARKDOWN_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v18_preflight.md"
)


def main() -> None:
    if REPORT_PATH.exists() or MARKDOWN_PATH.exists():
        raise RuntimeError("Supervised labeler v18 preflight already exists")
    ...
    if (
        ...
        raise RuntimeError("v18 registration and frozen split disagree")
    ...
    v18_groups = set(split["v18_reserved_group_ids"])
    v17_revealed_groups = set(split["v17_revealed_audit_group_ids"])
    v17_nonselected_groups = set(
    ...
    if (
        ...
        or model_groups & v18_groups
        or model_groups & v17_nonselected_groups
        or model_groups & prior_sealed_groups
        or not v17_revealed_groups <= training_groups
        or not replay_ids <= set(split["training_image_ids"])
        or len(audit) != 48
    ):
        raise RuntimeError("v18 model-data independence changed")
    ...
    if set(replay_weights.values()) != {12.0, 16.0}:
        raise RuntimeError("v18 error-replay weights changed")
    if (
        replay_weights["551"] != 12.0
        or replay_weights["857"] != 16.0
        or replay_weights["4187"] != 16.0
        or replay_weights["2262"] != 16.0
        or replay_weights["2580"] != 16.0
        or replay_weights["2941"] != 16.0
        or replay_weights["361"] != 16.0
        or replay_weights["4151"] != 16.0
    ):
        raise RuntimeError("v18 replay maximum-weight policy changed")
    ...
    payload = {
        ...
        "v18_reserved_groups_in_model_data": 0,
        "v17_nonselected_groups_in_model_data": 0,
        "prior_sealed_groups_in_model_data": 0,
        "v17_revealed_audit_groups_in_training": len(v17_revealed_groups),
    ...
    lines = [
        "# Supervised labeler v18 CPU preflight",
        "",
        ...
        "- Prior sealed, nonselected v17, and v18 groups in model data: **0**",
        "- Owner-approved v17 audit groups in training: **48**",
        "- Historic positive replay: **12.0**",
        "- v17 owner miss / false-positive replay: **16.0**",
