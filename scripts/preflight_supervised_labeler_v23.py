"""Verify v23 partitions, normalization, replay, and base model on CPU."""
...
REPORT_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v23_preflight.json"
MARKDOWN_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v23_preflight.md"
)


def main() -> None:
    if REPORT_PATH.exists() or MARKDOWN_PATH.exists():
        raise RuntimeError("Supervised labeler v23 preflight already exists")
    ...
    if (
        ...
        raise RuntimeError("v23 registration and frozen split disagree")
    ...
    v23_groups = set(split["v23_reserved_group_ids"])
    v22_revealed_groups = set(split["v22_revealed_audit_group_ids"])
    v22_nonselected_groups = set(
    ...
    prior_sealed_groups = {
        ...
        and key != "v22_nonselected_excluded_group_ids"
    ...
    if (
        ...
        or model_groups & v23_groups
        or model_groups & v22_nonselected_groups
        or model_groups & prior_sealed_groups
        or not v22_revealed_groups <= training_groups
        or not replay_ids <= set(split["training_image_ids"])
        or len(audit) != 48
    ):
        raise RuntimeError("v23 model-data independence changed")
    ...
    if not set(replay_weights.values()) <= {12.0, 28.0, 40.0}:
        raise RuntimeError("v23 error-replay weights changed")
    ...
    if (
        not {487, 93} <= owner_ids
        or not {2969, 972, 3405} <= hard_ids
        or any(
            replay_weights[str(image_id)] != 40.0
            for image_id in owner_ids
        )
        or any(
            replay_weights[str(image_id)] != 28.0
            for image_id in hard_ids - owner_ids
        )
        or any(
        ...
        raise RuntimeError("v23 replay maximum-weight policy changed")
    ...
    payload = {
        ...
        "v23_reserved_groups_in_model_data": 0,
        "v22_nonselected_groups_in_model_data": 0,
        "prior_sealed_groups_in_model_data": 0,
        "v22_revealed_audit_groups_in_training": len(v22_revealed_groups),
