"""Freeze exact v11 owner-review files before presenting them."""

from __future__ import annotations

import json

from scripts.record_supervised_labeler_v11_review import (
    REVIEW_MANIFEST_PATH,
    build_v11_registration,
)
...
def main() -> None:
    if REVIEW_MANIFEST_PATH.exists():
        raise RuntimeError("v11 owner-review manifest is already frozen")
    config = load_supervised_labeler_config(CONFIG_PATH)
    split = json.loads(SPLIT_PATH.read_text(encoding="utf-8"))
    report = json.loads(REPORT_PATH.read_text(encoding="utf-8"))
    registration = build_v11_registration(
        config=config,
        split=split,
        report=report,
    )
    payload = {
        "schema_version": 1,
        "status": "v11_owner_review_files_frozen",
