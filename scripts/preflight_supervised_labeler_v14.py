"""Verify v14 partitions, normalization, sampling, and base model on CPU."""
...
REPORT_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v14_preflight.json"
MARKDOWN_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v14_preflight.md"
)


def main() -> None:
    if REPORT_PATH.exists() or MARKDOWN_PATH.exists():
        raise RuntimeError("Supervised labeler v14 preflight already exists")
    ...
    if (
        ...
        raise RuntimeError("v14 registration and frozen split disagree")
    ...
    if (
        ...
        or (training_groups | calibration_groups) & v14_groups
        or (training_groups | calibration_groups) & v13_reserve_groups
        or (training_groups | calibration_groups) & v12_groups
        or not v13_primary_groups <= training_groups
        or len(audit) != 48
    ):
        raise RuntimeError("v14 model-data independence changed")
    ...
    replay_weights = {
        ...
        for image_id in split["owner_miss_replay_image_ids"]
    }
    if set(replay_weights.values()) != {
        float(sampling["owner_miss_replay_weight"])
    }:
        raise RuntimeError("v14 owner-miss replay weight changed")
    ...
    payload = {
        ...
        "owner_miss_replay_weights": replay_weights,
        "model": {
    ...
    lines = [
        "# Supervised labeler v14 CPU preflight",
        "",
        ...
        "- v12 / v13 sealed / v14 groups in model data: **0 / 0 / 0**",
        "- Approved v13 primary groups in training: **64**",
