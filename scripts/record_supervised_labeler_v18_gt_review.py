"""Record the v18 GT-only owner review and freeze its clean audit."""
...
OWNER_REVIEW_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v18_gt_owner_review.json"
)
AUDIT_PATH = (
...
PROBLEMS = {
    4: {
        ...
        "reason": (
            "The top-right green box covers an ordinary face with no visible "
            "safety hard hat."
        ),
    },
    12: {
        ...
        "decision": "AMBIGUOUS",
        "reason": (
            "The far-left target is a narrow clipped edge fragment and does "
            "not require a meaningful worn-helmeted-head box."
        ),
    },
    24: {
        "image_id": 4378,
        "decision": "AMBIGUOUS",
        "reason": (
            "The far-right target is a narrow clipped edge fragment and does "
            "not require a meaningful worn-helmeted-head box."
        ),
    },
    35: {
    ...
    *,
    ...
    for case in evidence["cases"]:
        ...
        if problem is not None and image_id != int(problem["image_id"]):
            raise RuntimeError(f"v18 problem cell {cell:02d} identity changed")
        decisions.append(
            {
                "cell": cell,
    ...
    review = {
        "schema_version": 1,
        "status": "v18_gt_only_primary_adjudicated_four_quarantines",
        ...
        "decisions": decisions,
        "owner_note": (
            "Quarantine complete image 04 because its top-right green box "
            "covers a face without a visible safety hard hat. Quarantine "
            "images 12, 24, and 35 because their clipped edge fragments do "
            "not require meaningful boxes. All other 60 primary GT-only "
            "cells are approved."
        ),
...
def build_adjudicated_audit(
    *,
    ...
    if Counter(decisions.values()) != Counter(
        {"PASS": 60, "AMBIGUOUS": 3, "DATASET_GT_FALSE_POSITIVE": 1}
    ):
        raise RuntimeError("v18 owner review decisions changed")
    ...
    if dict(selected_per_stratum) != quotas:
        raise RuntimeError("Valid v18 primary cases do not satisfy quotas")
    ...
    manifest = {
        "schema_version": 1,
        "status": "v18_adjudicated_audit_frozen_before_training",
        "experiment_id": str(pool["experiment_id"]),
        "source_split": "Train",
    ...
    if (
        ...
        raise RuntimeError("v18 adjudicated audit violates frozen boundaries")
...
def main() -> None:
    ...
    parser.add_argument(
        "--quarantine-cells",
        required=True,
        help="Must be exactly 4,12,24,35.",
    )
    ...
    if cells != set(PROBLEMS):
        raise RuntimeError(
            "Explicit --quarantine-cells 4,12,24,35 is required"
        )
    if OWNER_REVIEW_PATH.exists() or AUDIT_PATH.exists():
        raise RuntimeError("v18 GT-only owner evidence already exists")
