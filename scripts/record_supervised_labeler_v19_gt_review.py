"""Record the v19 GT-only owner review and freeze its clean audit."""
...
OWNER_REVIEW_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v19_gt_owner_review.json"
)
AUDIT_PATH = (
...
PROBLEMS = {
    36: {
        ...
        "decision": "AMBIGUOUS",
        "reason": (
            "The far-left green box covers a tiny clipped edge fragment. It "
            "is too small and incomplete to require a meaningful worn-"
            "helmeted-head box."
        ),
    }
}
...
def build_owner_review(
    *,
    ...
    """Build the exact one-quarantine decision over all 64 primary cases."""
    ...
    for case in evidence["cases"]:
        ...
        if problem is not None and (
            ...
            raise RuntimeError(f"v19 problem cell {cell:02d} identity changed")
        decisions.append(
            {
                "cell": cell,
                "group_id": group_id,
    ...
    review = {
        "schema_version": 1,
        "status": "v19_gt_only_primary_adjudicated_one_quarantine",
        ...
        "decisions": decisions,
        "owner_note": (
            "Quarantine complete image 36 because its far-left target is a "
            "tiny clipped edge fragment that does not require a meaningful "
            "box. All other 63 primary GT-only cells are approved."
        ),
...
def build_adjudicated_audit(
    *,
    ...
    if Counter(decisions.values()) != Counter({"PASS": 63, "AMBIGUOUS": 1}):
        raise RuntimeError("v19 owner review decisions changed")
    ...
    if dict(selected_per_stratum) != quotas:
        raise RuntimeError("Valid v19 primary cases do not satisfy quotas")
    ...
    manifest = {
        "schema_version": 1,
        "status": "v19_adjudicated_audit_frozen_before_training",
        "experiment_id": str(pool["experiment_id"]),
        "source_split": "Train",
    ...
    if (
        ...
        raise RuntimeError("v19 adjudicated audit violates frozen boundaries")
...
def main() -> None:
    ...
    if cells != set(PROBLEMS):
        raise RuntimeError("Explicit --quarantine-cells 36 is required")
    if OWNER_REVIEW_PATH.exists() or AUDIT_PATH.exists():
        raise RuntimeError("v19 GT-only owner evidence already exists")
