"""Record kuotunyu's decision on exact frozen v8 review evidence."""
...
OUTPUT_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v8_human_review.json"
REVIEW_MANIFEST_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v8_review_manifest.json"
)
...
def _record(path: Path) -> dict[str, str]:
    if not path.is_file():
        raise RuntimeError(f"Frozen v8 review evidence is missing: {path}")
    return {
...
def build_v8_registration(
    *,
    config: dict[str, Any],
    split: dict[str, Any],
    report: dict[str, Any],
) -> dict[str, Any]:
    """Construct and verify the exact v8 numeric-pass review registration."""

    if (
        config.get("experiment_id") != "supervised_labeler_v8"
        ...
        raise RuntimeError("v8 must pass every numeric gate before human review")
    ...
    if (
        ...
        raise RuntimeError("Exact v8 audit box evidence changed")
    registration = {
...
def main() -> None:
    ...
    fresh_registration = build_v8_registration(
        config=config,
        split=split,
        report=report,
    )
    if not REVIEW_MANIFEST_PATH.is_file():
        raise RuntimeError("Frozen v8 owner-review manifest is missing")
    ...
    if (
        ...
        or review_manifest.get("status") != "v8_owner_review_files_frozen"
        or review_manifest.get("registration") != fresh_registration
    ):
        raise RuntimeError("Frozen v8 owner-review manifest changed")
