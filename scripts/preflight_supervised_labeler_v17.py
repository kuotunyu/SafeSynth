"""Verify v17 partitions, normalization, replay sampling, and base model on CPU."""
...
REPORT_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v17_preflight.json"
MARKDOWN_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v17_preflight.md"
)


def main() -> None:
    if REPORT_PATH.exists() or MARKDOWN_PATH.exists():
        raise RuntimeError("Supervised labeler v17 preflight already exists")
    ...
    if (
        ...
        raise RuntimeError("v17 registration and frozen split disagree")
    ...
    v17_groups = set(split["v17_reserved_group_ids"])
    v16_revealed_groups = set(split["v16_revealed_audit_group_ids"])
    v16_nonselected_groups = set(
    ...
    if (
        ...
        or model_groups & v17_groups
        or model_groups & v16_nonselected_groups
        or model_groups & prior_sealed_groups
        or not v16_revealed_groups <= training_groups
        or not replay_ids <= set(split["training_image_ids"])
        or len(audit) != 48
    ):
        raise RuntimeError("v17 model-data independence changed")
    ...
    if set(replay_weights.values()) != {12.0}:
        raise RuntimeError("v17 error-replay weights changed")
    if replay_weights["361"] != 12.0 or replay_weights["4151"] != 12.0:
        raise RuntimeError("v17 overlapping replay weights stacked")
    ...
    payload = {
        ...
        "overlapping_replay_image_ids": [361, 4151],
        ...
        "v17_reserved_groups_in_model_data": 0,
        "v16_nonselected_groups_in_model_data": 0,
        "prior_sealed_groups_in_model_data": 0,
        "v16_revealed_audit_groups_in_training": len(v16_revealed_groups),
    ...
    lines = [
        "# Supervised labeler v17 CPU preflight",
        "",
        ...
        "- Prior sealed, nonselected v16, and v17 groups in model data: **0**",
        "- Owner-approved v16 audit groups in training: **48**",
