"""Record kuotunyu's final review of the frozen v22 model audit pages."""
...
MISSED_CELLS = [6, 34]
FALSE_POSITIVE_CELLS = [4, 28, 32]
UNWORN_HELMET_FALSE_POSITIVE_CELLS = [32]
EXPECTED_IMAGE_IDS = {
    4: 2969,
    6: 487,
    28: 972,
...
def build_review(
    *,
    ...
    """Build the exact five-cell owner rejection."""
    ...
    problem_cases = [
        {
            ...
            "detail": (
                "unworn_helmet_without_head"
                if cell in UNWORN_HELMET_FALSE_POSITIVE_CELLS
                else None
            ),
        }
    ...
    review = {
        ...
        "categories": {
            ...
            "unworn_helmet_without_head_false_positive_cells": (
                UNWORN_HELMET_FALSE_POSITIVE_CELLS
            ),
        ...
        "accepted_exceptions": [],
        "review_note": (
            "kuotunyu identified magenta predictions on wrong objects in "
            "cells 04 and 28, an unworn helmet without a head in cell 32, "
            "and missed worn helmeted heads in cells 06 and 34. All other "
            "reviewed cells were accepted. The required zero-problem visual "
...
def build_diagnosis(
    *,
        ...
        cell_diagnoses.append(
            {
                ...
                "owner_detail": (
                    "unworn_helmet_without_head"
                    if cell_number
                    in UNWORN_HELMET_FALSE_POSITIVE_CELLS
                    else None
                ),
    ...
    if aggregate != {
        ...
        raise RuntimeError("v22 reviewed-cell diagnosis changed")

    diagnosis = {
        "schema_version": 1,
        "status": "v22_owner_review_diagnosed_without_new_inference",
        "experiment_id": "supervised_labeler_v22",
        ...
        "root_cause_summary": {
            "misses": (
                "The two owner-reported positive cells contain two unmatched "
                "ground-truth worn helmeted heads at the frozen threshold."
            ),
            "false_positives": (
                "The three owner-reported empty-GT cells each contain one "
                "model prediction. Cell 32 is specifically an unworn helmet "
                "without a head. The matcher also counts one unmatched "
                "prediction in owner-reported miss cell 34."
            ),
            "rendering_bug": False,
        },
        "future_intervention_constraint": (
            "Do not change the v22 threshold or geometry after audit. Replay "
            "miss image IDs 487 and 93 as positives and false-positive image "
            "IDs 2969, 972, and 3405 as hard negatives only in a "
            "preregistered fresh base-initialized model with a new "
            "independent audit."
        ),
...
def main() -> None:
    if OUTPUT_PATH.exists() or DIAGNOSIS_PATH.exists():
        raise RuntimeError("v22 owner review evidence already exists")
    ...
    if (
        ...
        raise RuntimeError("Configured v22 model outcome changed")
