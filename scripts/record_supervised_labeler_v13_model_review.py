"""Record kuotunyu's review of the frozen v13 model audit pages."""
...
OUTPUT_PATH = (
    PROJECT_ROOT
    / "reports"
    / "supervised_labeler_v13_model_human_review.json"
)
...
def _verified_manifest() -> dict[str, Any]:
    ...
    if (
        ...
        raise RuntimeError("Frozen v13 model-review manifest changed")
    return manifest
...
def build_review(
    *,
    ...
    """Build the exact zero-false-positive, three-miss owner rejection."""
    ...
    if (
        ...
        raise RuntimeError("Owner decision does not match exact v13 evidence")
    evidence_by_cell = {
        ...
        "problem_cases": [
            {
                ...
                "category": "model_missed_helmeted_head",
            }
            for cell in problem_cells
        ],
        "review_note": (
            "Most v13 boxes were good. Real worn-helmeted heads were missed "
            "in cells 03, 22, and 34. No model false-positive cell was "
            "reported."
        ),
...
def main() -> None:
    ...
    if (
        ...
        raise RuntimeError("Configured v13 model outcome changed")
    review = build_review(
        manifest=manifest,
        evidence=evidence,
