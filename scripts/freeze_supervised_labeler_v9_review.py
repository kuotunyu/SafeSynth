"""Freeze exact v9 owner-review files before presenting them."""

from __future__ import annotations

import json

from scripts.record_supervised_labeler_v9_review import (
    REVIEW_MANIFEST_PATH,
    build_v9_registration,
)
...
def main() -> None:
    if REVIEW_MANIFEST_PATH.exists():
        raise RuntimeError("v9 owner-review manifest is already frozen")
    config = load_supervised_labeler_config(CONFIG_PATH)
    split = json.loads(SPLIT_PATH.read_text(encoding="utf-8"))
    report = json.loads(REPORT_PATH.read_text(encoding="utf-8"))
    registration = build_v9_registration(
        config=config,
        split=split,
        report=report,
    )
    payload = {
        "schema_version": 1,
        "status": "v9_owner_review_files_frozen",
