"""Verify v20 partitions, normalization, replay, and base model on CPU."""
...
REPORT_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v20_preflight.json"
MARKDOWN_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v20_preflight.md"
)


def main() -> None:
    if REPORT_PATH.exists() or MARKDOWN_PATH.exists():
        raise RuntimeError("Supervised labeler v20 preflight already exists")
    ...
    if (
        ...
        raise RuntimeError("v20 registration and frozen split disagree")
    ...
    v20_groups = set(split["v20_reserved_group_ids"])
    v19_revealed_groups = set(split["v19_revealed_audit_group_ids"])
    v19_nonselected_groups = set(
        split["v19_nonselected_excluded_group_ids"]
    )
    prior_sealed_groups = (
        set(split["v18_nonselected_excluded_group_ids"])
    ...
    if (
        ...
        or model_groups & v20_groups
        or model_groups & v19_nonselected_groups
        or model_groups & prior_sealed_groups
        or not v19_revealed_groups <= training_groups
        or not replay_ids <= set(split["training_image_ids"])
        or len(audit) != 48
    ):
        raise RuntimeError("v20 model-data independence changed")
    ...
    if not set(replay_weights.values()) <= {12.0, 24.0}:
        raise RuntimeError("v20 error-replay weights changed")
    if (
        replay_weights["551"] != 12.0
        or any(
            replay_weights[str(image_id)] != 24.0
            for image_id in (118, 857, 3117, 3651, 4187, 4452, 4924)
        )
        or any(
            replay_weights[str(image_id)] != 24.0
            for image_id in (1241, 2262, 2580, 2910, 2941, 4618)
        )
        or replay_weights["361"] != 24.0
        or replay_weights["4151"] != 24.0
    ):
        raise RuntimeError("v20 replay maximum-weight policy changed")
    ...
    payload = {
        ...
        "v20_reserved_groups_in_model_data": 0,
        "v19_nonselected_groups_in_model_data": 0,
        "prior_sealed_groups_in_model_data": 0,
        "v19_revealed_audit_groups_in_training": len(v19_revealed_groups),
    ...
    lines = [
        "# Supervised labeler v20 CPU preflight",
        "",
        ...
        "- Prior sealed, nonselected v19, and v20 groups in model data: **0**",
        "- Owner-reviewed v19 audit groups in training: **48**",
        "- Historic positive replay: **12.0**",
        "- v19 owner-miss replay: **24.0**",
        "- v19 hard-negative replay: **24.0**",
