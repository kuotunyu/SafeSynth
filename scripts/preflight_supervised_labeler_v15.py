"""Verify v15 partitions, normalization, replay sampling, and base model on CPU."""
...
REPORT_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v15_preflight.json"
MARKDOWN_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v15_preflight.md"
)


def main() -> None:
    if REPORT_PATH.exists() or MARKDOWN_PATH.exists():
        raise RuntimeError("Supervised labeler v15 preflight already exists")
    ...
    if (
        ...
        raise RuntimeError("v15 registration and frozen split disagree")
    ...
    v15_groups = set(split["v15_reserved_group_ids"])
    ...
    if (
        ...
        raise RuntimeError("v15 model-data independence changed")
    ...
    if set(replay_weights.values()) != {12.0}:
        raise RuntimeError("v15 error-replay weights changed")
    if replay_weights["361"] != 12.0:
        raise RuntimeError("v15 overlapping replay weights stacked")
    ...
    payload = {
        ...
        "v15_reserved_groups_in_model_data": 0,
    ...
    lines = [
        "# Supervised labeler v15 CPU preflight",
        "",
        ...
        (
            "- v12 / v13 sealed / v14 sealed / v15 groups in model data: "
            "**0 / 0 / 0 / 0**"
        ),
        ...
        "- v15 sealed-reserve pixels read: **0**",
