"""Record kuotunyu's decision on exact frozen v9 review evidence."""
...
OUTPUT_PATH = PROJECT_ROOT / "reports" / "supervised_labeler_v9_human_review.json"
REVIEW_MANIFEST_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v9_review_manifest.json"
)
...
def _record(path: Path) -> dict[str, str]:
    if not path.is_file():
        raise RuntimeError(f"Frozen v9 review evidence is missing: {path}")
    return {
...
def build_v9_registration(
    *,
    config: dict[str, Any],
    split: dict[str, Any],
    report: dict[str, Any],
) -> dict[str, Any]:
    """Construct and verify the exact v9 numeric-pass review registration."""

    if (
        config.get("experiment_id") != "supervised_labeler_v9"
        ...
        raise RuntimeError("v9 must pass every numeric gate before human review")
    ...
    if (
        ...
        raise RuntimeError("Exact v9 audit box evidence changed")
    registration = {
...
def main() -> None:
    ...
    fresh_registration = build_v9_registration(
        config=config,
        split=split,
        report=report,
    )
    if not REVIEW_MANIFEST_PATH.is_file():
        raise RuntimeError("Frozen v9 owner-review manifest is missing")
    ...
    if (
        ...
        or review_manifest.get("status") != "v9_owner_review_files_frozen"
        or review_manifest.get("registration") != fresh_registration
    ):
        raise RuntimeError("Frozen v9 owner-review manifest changed")
