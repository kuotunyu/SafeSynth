"""Record the v21 GT-only owner review and freeze its clean audit."""
...
OWNER_REVIEW_PATH = (
    PROJECT_ROOT / "reports" / "supervised_labeler_v21_gt_owner_review.json"
)
AUDIT_PATH = (
...
PROBLEMS = {
    11: {
        ...
        "reason": (
            "The left green box covers a clipped bare face with no visible "
            "safety hard hat."
        ),
    },
    33: {
        ...
        "decision": "DATASET_GT_FALSE_POSITIVE",
        "reason": (
            "The far-left green box covers a clipped person without a visible "
            "safety hard hat."
        ),
    },
    35: {
        ...
        "decision": "DATASET_GT_FALSE_POSITIVE",
        "reason": (
            "The middle green box covers a bare head; no safety hard hat is "
            "visible."
        ),
    },
}
...
def build_owner_review(
    *,
    ...
    """Build the exact three-quarantine decision over all 64 primary cases."""
    ...
    for case in evidence["cases"]:
        ...
        if problem is not None and (
            ...
            raise RuntimeError(f"v21 problem cell {cell:02d} identity changed")
        decisions.append(
            {
                "cell": cell,
                "group_id": group_id,
    ...
    review = {
        "schema_version": 1,
        "status": "v21_gt_only_primary_adjudicated_three_quarantines",
        ...
        "categories": {
            "dataset_gt_false_positive_cells": [11, 33, 35],
        ...
        "decisions": decisions,
        "owner_note": (
            "Quarantine complete images 11 and 33 because their left clipped "
            "green boxes cover people with no visible safety hard hat. "
            "Quarantine complete image 35 because its middle green box covers "
            "a bare head. All other 61 primary GT-only cells are approved."
        ),
...
def build_adjudicated_audit(
    *,
    ...
    if Counter(decisions.values()) != Counter(
        {"PASS": 61, "DATASET_GT_FALSE_POSITIVE": 3}
    ):
        raise RuntimeError("v21 owner review decisions changed")
    ...
    if dict(selected_per_stratum) != quotas:
        raise RuntimeError("Valid v21 primary cases do not satisfy quotas")
    ...
    manifest = {
        "schema_version": 1,
        "status": "v21_adjudicated_audit_frozen_before_training",
        "experiment_id": str(pool["experiment_id"]),
        "source_split": "Train",
    ...
    if (
        ...
        raise RuntimeError("v21 adjudicated audit violates frozen boundaries")
...
def main() -> None:
    ...
    if cells != set(PROBLEMS):
        raise RuntimeError("Explicit --quarantine-cells 11,33,35 is required")
    if OWNER_REVIEW_PATH.exists() or AUDIT_PATH.exists():
        raise RuntimeError("v21 GT-only owner evidence already exists")

    config = yaml.safe_load(CONFIG_PATH.read_text(encoding="utf-8"))
    if config["status"] != "gt_only_primary_review_pending_owner":
        raise RuntimeError("v21 GT-only owner review is not pending")
    pool = _verified_json(
        POOL_PATH,
