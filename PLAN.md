...
- [x] **M9** hard negative 挖料與程序生成 ＋ Spike H6 ＋ **使用者人工簽核**
  ...
  - **驗證於**：`c7514f4` @ 2026-07-27（程式與三層防護）；
    `978bfd7` @ 2026-07-31（素材庫落地並接入 compositor）。
    H6 64 格由 kuotunyu 簽核 **0/64 真正安全帽**，簽核綁定 exact-grid SHA256。
    ⚠️ **素材改為 240 個全程序生成，挖料完全不用**（[ADR-012](docs/decisions.md#adr-012)）——
    目視 16 個通過尺寸篩選的挖料 cutout，發現全是細長背景色帶與平坦紋理方塊，
    HSV＋輪廓找到的是顏色區域而非物件。程序生成則渲染在安全帽典型尺寸
    （min_side 24–79、中位數 50；真實安全帽 median 34／p90 74）。
    冒煙驗證：12 張中 11 張通過、32 個 distractor 貼上、**產生 0 個標註**、
    與任何標註的 IoU 最大值 **0.0000**（FILT-10 上限 0.02）、真實標註數不變式成立
